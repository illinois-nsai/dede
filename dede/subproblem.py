...
class Subproblem(CpProblem):
    ...
    def __init__(
        ...
        self.var_id_pos_list = sorted(list(var_id_pos_set1)) + sorted(list(var_id_pos_set2))
        ...
        var_id_to_new_var, constrs_var_attr = self.get_var_id_to_new_var(objective_expr, constrs_gp)

        # objective
        self.obj_expr_old = replace_variables(objective_expr, var_id_to_new_var)

        # lambda for original constraints
        self.f1 = cp.hstack(
            [replace_variables(constr.expr, var_id_to_new_var).flatten() for constr in constrs_gp]
        )
    ...
    def get_var_id_to_new_var(
        ...
        if attr_idx["boolean"]:
            constrs_var_attr.append(
                self.var[attr_idx["boolean"]] == cp.Variable(len(attr_idx["boolean"]), boolean=True)
            )
        if attr_idx["integer"]:
            constrs_var_attr.append(
                self.var[attr_idx["integer"]] == cp.Variable(len(attr_idx["integer"]), integer=True)
            )

...
def get_shardLoads(num_shards, randomNum):
    ...
    for shardNum in range(num_shards):
        load = (
            int(round(1000000.0 * (1.0 / math.pow(shardNum / num_shards * 1024 + 1, zipfValue))))
            / 10000.0
        )
...
def benchmark(args):
    df = DeDeFormulation(
        args.num_servers, args.num_shards, args.epsilon, args.memory_limit, args.search_limit
    )
    # warm-up
    ...
    with open("dede-form.csv", "a") as results:
        ...
        for i, randomNum in enumerate(randomNums[args.warmup :]):
            ...
            obj = df.solve(
                shardLoads, num_cpus=args.num_cpus, rho=args.rho, num_iter=args.admm_steps
            )

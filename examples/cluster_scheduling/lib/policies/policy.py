...
class PolicyWithPacking(Policy):
    ...
    def get_base_constraints(self, x, single_job_ids, scale_factors_array, relevant_combinations):
        ...
        constraints = [
            x >= 0,
            cp.sum(cp.multiply(scale_factors_array, x), axis=0) <= np.array(self._num_workers),
        ]
    ...
    def convert_job_type_allocation(self, allocation, job_id_to_job_type_key):
        ...
        for i, job_id in enumerate(job_ids):
            ...
            for other_job_id in job_ids[i + 1 :]:
                ...
                for worker_type in worker_types:
                    current_job_type_allocation = job_type_allocation[worker_type][job_type_key][
                        other_job_type_key
                    ]
                    if current_job_type_allocation > 0.0:
                        if job_type_key == other_job_type_key:
                            current_job_type_allocation -= allocation[job_id][worker_type][
                                job_type_key
                            ]
...
class MaxMinFairnessPolicyWithPerf(Policy):
    ...
    def get_allocation(
        self, unflattened_throughputs, scale_factors, unflattened_priority_weights, cluster_spec
    ):
        throughputs, index = super().flatten(unflattened_throughputs, cluster_spec)
        if throughputs is None:
            return None
        ...
        priority_weights = np.multiply(
            priority_weights.reshape((m, 1)), 1.0 / proportional_throughputs.reshape((m, 1))
        )
        ...
        objective = cp.Maximize(
            cp.min(
                cp.sum(
                    cp.multiply(
                        np.multiply(
                            throughputs * priority_weights.reshape((m, 1)), scale_factors_array
                        ),
                        x,
                    ),
                    axis=1,
...
class MaxProportionalFairness(Policy):
    ...
    def get_allocation(
        ...
        fix_x = (
            fix_x
            / np.maximum(
                (scale_factors_array * fix_x).sum(0) / (np.array(self._num_workers) + EPS), 1
            )[None, :]
        )
...
class ProportionalPolicy(Policy):
    ...
    def get_throughputs(self, throughputs, index, cluster_spec):
        ...
        proportional_throughputs = np.sum(np.multiply(throughputs, x_proportional), axis=1).reshape(
            (m, 1)
        )
    ...
    def _get_allocation(self, throughputs, index, cluster_spec):
        ...
        x = np.array(
            [[cluster_spec[worker_type] / m for worker_type in worker_types] for i in range(m)]
        )
...
class GandivaPolicy(PolicyWithPacking):
    ...
    def _get_allocation(self, job_combinations_to_schedule, index, scale_factors, cluster_spec):
    ...
    def get_allocation(
        ...
        if num_workers_requested <= num_workers_available:
            ...
            for single_job_id in self._assigned_combinations:
                job_combinations_to_schedule.add(self._assigned_combinations[single_job_id][0])
        ...
        results = np.multiply(
            np.multiply(throughputs * priority_weights.reshape((m, 1)), scale_factors_array), x
        ).sum(1)

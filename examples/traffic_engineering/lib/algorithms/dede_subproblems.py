...
class SubproblemsWrap():
    ...
    def __init__(
            self, objective, num_nodes, num_edges, idx_r, idx_d,
        ...
        for i in range(len(idx_r)):
            ...
            self.probs_r.append(SubproblemR(
                self._objective, num_nodes, num_edges, (0, idx), constrs_gp, rho))
        self.probs_d = []
        for i in range(len(idx_d)):
            idx, constrs_gp = idx_d[i], constrs_gps_d[i]
            self.probs_d.append(SubproblemD(
                self._objective, num_nodes, num_edges, (1, idx), constrs_gp, rho))
...
class SubproblemR(CpProblem):
    def __init__(self, objective, num_nodes, num_edges, idx, constrs_gp, rho):
        ...
        if self._objective_name == Objective.TOTAL_FLOW:
            ...
            self.l2 = cp.Parameter(
                self.f2.shape, value=np.zeros(self.f2.shape))

            super(SubproblemR, self).__init__(
                cp.Minimize(
                    self.rho / 2 * cp.sum_squares(self.f1 + self.l1) +
                    self.rho / 2 * cp.sum_squares(self.var) - self.rho * self.var @ self.param + self.rho * self.var @ self.l2)
            ...
            # lambda for original constraints
            self.f1 = self.var[:-1].sum() + self.s - \
                self.capacity * self.var[-1]
            self.l1 = cp.Parameter(value=0.0)
            # lambda for x = z
            self.f2 = self.var - self.param
            self.l2 = cp.Parameter(
...
class SubproblemD(CpProblem):
    def __init__(self, objective, num_nodes, num_edges, idx, constrs_gp, rho):
        ...
        for dst, demand, idx_paths in zip(*constrs_gp):
            for idx_p in idx_paths:
                ...
                self.m_all_e_all_p += [[idx_e, self.num_paths]
                                       for idx_e in idx_p]
                self.num_paths += 1
        e_to_shorter_e_dict = {}
        self.num_edges = num_edges
        ...
        self.m_all_d_all_p = coo_matrix((np.ones(len(self.m_all_d_all_p)), np.array(
            self.m_all_d_all_p).T), shape=(num_nodes, self.num_paths))
        self.m_all_e_all_p = [[e_to_shorter_e_dict[idx_e], p]
                              for idx_e, p in self.m_all_e_all_p]
        self.m_all_e_all_p = coo_matrix((np.ones(len(self.m_all_e_all_p)), np.array(
            self.m_all_e_all_p).T), shape=(len(e_to_shorter_e_dict), self.num_paths))
        ...
        if self._objective_name == Objective.TOTAL_FLOW:
            ...
            self.l1 = cp.Parameter(
                self.f1.shape, value=np.zeros(self.f1.shape))
            # lambda for x = z
            ...
            for dst, demand, idx_paths in zip(*constrs_gp):
                ...
                for idx_e in idx_paths[0]:
                    ...
                    idx_e_dict[idx_e] = np.arange(
                        self.num_rest_paths, self.num_rest_paths + len(idx_paths) - 1).tolist()
                ...
                for idx_p in idx_paths[1:]:
                    ...
                    for idx_e in idx_p:
                        if idx_e not in idx_e_dict:
                            self.m_all_e_rest_p.append(
                                [idx_e, self.num_rest_paths, 1])
                        else:
            ...
            self.m_all_e_all_d = [[e_to_shorter_e_dict[idx_e], d]
                                  for idx_e, d in self.m_all_e_all_d]
            self.m_all_e_all_d = coo_matrix((np.ones(len(self.m_all_e_all_d)), np.array(
                self.m_all_e_all_d).T), shape=(len(e_to_shorter_e_dict), num_nodes))
            self.demands = cp.Parameter(num_nodes)
            self.demands.value = np.array(constrs_gp[1])
            ...
            if self.num_rest_paths > 0:
                self.m_all_d_rest_p = coo_matrix((np.ones(len(self.m_all_d_rest_p)), np.array(
                    self.m_all_d_rest_p).T), shape=(num_nodes, self.num_rest_paths))
                self.m_all_e_rest_p = np.array(
                    [[e_to_shorter_e_dict[idx_e], p, val] for idx_e, p, val in self.m_all_e_rest_p]).T
                self.m_all_e_rest_p = coo_matrix(
                    (self.m_all_e_rest_p[-1], self.m_all_e_rest_p[:-1]), shape=(len(e_to_shorter_e_dict), self.num_rest_paths))
    ...
    def fix(self, param_value=None):
        ...
        if self._objective_name == Objective.TOTAL_FLOW:
            ...
            self.var_fix = self.var.value / \
                (np.maximum(self.m_all_d_all_p @ self.var.value /
                 (self.demands.value + EPS), 1) @ self.m_all_d_all_p)
            ...
            self.var_fix /= coo_matrix(([edge_oversubscribe[i] for i in self.m_all_e_all_p.row], (self.m_all_e_all_p.row,
                                       self.m_all_e_all_p.col)), shape=self.m_all_e_all_p.shape).max(axis=0).toarray().reshape(-1)
            # no violation
            ...
            if self.num_rest_paths > 0:
                ...
                self.all_var_fix[self.first_p_all_p_list] = np.maximum(np.delete(
                    self.demands.value - self.m_all_d_rest_p @ self.var_fix, self.id[1]), 0)
                # assume all the edge is at max utilization
                edge_oversubscribe = self.m_all_e_all_p @ self.all_var_fix / \
                    (param_value[self.idx_e_list] + EPS)
                self.all_var_fix /= coo_matrix(([edge_oversubscribe[i] for i in self.m_all_e_all_p.row], (self.m_all_e_all_p.row,
                                               self.m_all_e_all_p.col)), shape=self.m_all_e_all_p.shape).max(axis=0).toarray().reshape(-1) + EPS
                # remove demand over-allocation
                self.var_fix = self.all_var_fix[self.rest_p_all_p_list] / (
                    self.m_all_d_all_p @ self.all_var_fix / (self.demands.value + EPS) @ self.m_all_d_rest_p + EPS)
                res[self.idx_e_list] = self.m_all_e_rest_p @ self.var_fix + \
                    self.demands_on_edge.value
                # no violation

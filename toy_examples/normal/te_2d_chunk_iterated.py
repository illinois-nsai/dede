...
#         lhs = x[:, 1::2]
#         rhs = np.arange(lhs.size).reshape(lhs.shape)
#         resource_constraints = [lhs >= rhs]
...
#     plt.grid(True)
#     plt.tight_layout()
#     plt.savefig("te_2d_chunk_result_42.png")
...
random.seed(25)
log_path = "te_2d_chunk.log"

def opt():
    index = []
    dede_list = []
    cvxpy_list = []
    for size in range(100, 1001, 100):  # small sizes for correctness
        print("Size: ", size)
        index.append(size)
        np.set_printoptions(precision=3, suppress=True)
        N, M = size, size
        ...
        x = dd.Variable((N, M), nonneg=True)
        param = dd.Parameter(N, value=[round(random.uniform(1, 3), 1) for _ in range(N)])

        lhs = x[:, 1::size /2]
        rhs = np.arange(lhs.size).reshape(lhs.shape)
        resource_constraints = [lhs >= rhs]
        demand_constraints = [x.sum(0) <= size * 3]

        w = np.random.randint(1, N + 10, size=(N, M))
        objective = dd.Maximize(dd.sum(dd.multiply(x, w)))
        prob = dd.Problem(objective, resource_constraints, demand_constraints)
        ...
        lhs_c = xc[:, 1::size /2]
        rhs_c = np.arange(lhs_c.size).reshape(lhs_c.shape)
        resource_constraintsc = [lhs_c >= rhs_c]
    ...
    plt.xlabel("Problem Size (N = M)")
    plt.ylabel("Objective Value")
    plt.title("Performance of DEDE vs CVXPY (2D Chunk Constraint)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("te_2d_chunk_result.png")

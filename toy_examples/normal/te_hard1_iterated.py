...
#         resource_constraints = [x.sum(1) >= np.arange(size, size + size)]
#         demand_constraints = [x.sum(0) <= size * 3]
#         resource_constraintsc = [cp.sum(xc, axis=1) >= np.arange(size, size + size)]
...
#         #result_cvxpy = prob.solve(enable_dede=False, verbose = True)
...
#     plt.grid(True)
#     plt.tight_layout()
#     plt.savefig("te_hard1_result_42.png")
...
def opt():
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("te_hard1_result.png")
